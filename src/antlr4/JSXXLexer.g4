...
TagInsideName
  : HTMLNameStartChar HTMLNameChar*
  ;
...
// mode for attribute value
mode ATTR_VAL;

AttrVal
  : [ ]* AttrValContent    -> popMode
  ;

AttrValContent
  : AttrDoubleQuoteString
  | AttrSingleQuoteString
  ;
...
ExprClose
  ...
  {
    this.popMode();
    if (this._mode == JSXXLexer.ATTR_VAL) {
      this.popMode();
    }
  }
  ;
...
ExprMultiLineComment
  : '/*' .*? '*/'                             -> channel(HIDDEN)
  ;

ExprSingleLineComment
  : '//' ~[\r\n\u0085\u2028\u2029]*           -> channel(HIDDEN)
  ;
